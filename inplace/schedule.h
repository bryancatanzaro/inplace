...
struct smem {
    static const int lim = SM::smem / sizeof(T);
...
struct schedule<float, sm_35> {
    typedef cons<smem<float, sm_35>, cons<reg<5, 256>, cons<reg<5, 384>, cons<reg<5, 512>, cons<reg<20, 256>, cons<reg<20, 384>, cons<reg<20, 512>, cons<reg<37, 256>, cons<reg<37, 384>, cons<reg<37, 512>, cons<reg<77, 256>, cons<reg<77, 384>, cons<reg<77, 512>, cons<reg<100, 256>, cons<reg<100, 384>, cons<reg<100, 512>, memory> > > > > > > > > > > > > > > > type;
};

template<>
struct schedule<double, sm_35> {
    typedef cons<smem<double, sm_35>, cons<reg<4, 256>, cons<reg<4, 384>, cons<reg<4, 512>, cons<reg<11, 256>, cons<reg<11, 384>, cons<reg<11, 512>, cons<reg<18, 256>, cons<reg<18, 384>, cons<reg<18, 512>, cons<reg<34, 256>, cons<reg<34, 384>, cons<reg<34, 512>, cons<reg<51, 256>, cons<reg<51, 384>, cons<reg<51, 512>, cons<reg<83, 256>, cons<reg<115, 256>, memory> > > > > > > > > > > > > > > > > > type;
};


template<>
struct schedule<float, sm_20> {
    typedef cons<smem<float, sm_20>, cons<reg<5, 256>, memory> > type;

};

template<>
struct schedule<double, sm_20> {
    typedef cons<smem<double, sm_20>, cons<reg<5, 256>, memory> > type;
};
